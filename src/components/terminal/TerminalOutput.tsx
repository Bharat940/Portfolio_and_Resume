...
const parseAnsi = (text: string) => {
  ...
  return parts.map((part, i) => {
    if (part.startsWith("\x1b[")) {
      ...
      else if (part === "\x1b[1;33m")
        currentColor = "text-ctp-yellow font-bold";
      return null;
    }
    return (
        ...
        return (
          <div
            ...
            className={`leading-relaxed animate-in fade-in slide-in-from-left-2 duration-300 ${
              isArt
                ? "whitespace-pre overflow-x-auto pb-1 scrollbar-none"
                : entry.type === "ai"
                  ? "whitespace-normal"
                  : "whitespace-pre-wrap"
            } ${
              entry.type === "input"
                ...
                : entry.type === "error"
                  ? "text-ctp-red"
                  : entry.type === "ai"
                    ? "text-ctp-green border-l-2 border-ctp-green/10 pl-3 md:pl-4 mb-2"
                    : isGreeting
                      ? "text-ctp-mauve leading-[1.1] font-bold"
                      : "text-ctp-blue"
            }`}
          >
