  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col justify-end p-8 overflow-hidden">
        <m.div variants={contentVariants} className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            {project.tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="px-2.5 py-1 bg-ctp-mauve/10 border border-ctp-mauve/30 rounded-lg font-mono text-[10px] md:text-xs uppercase text-ctp-mauve font-bold"
              >
                {tag}
              </span>
            ))}
