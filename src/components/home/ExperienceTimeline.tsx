            ...
            >
              ...
              <div className={`w-full md:w-1/2 pl-16 md:pl-0 ${idx % 2 === 0 ? "md:pr-12 md:text-right" : "md:pl-12 md:text-left"
                ...
                <div className="bg-card border border-border/50 p-6 rounded-2xl shadow-xl hover:border-ctp-peach/50 transition-colors group">
                  <div className={`flex items-center gap-3 mb-2 ${idx % 2 === 0 ? "md:justify-end" : "md:justify-start"}`}>
                    <span className="p-2 bg-ctp-surface0 rounded-lg text-ctp-peach">
                      {item.type === 'work' ? <Briefcase className="w-4 h-4" /> : 
                       item.type === 'education' ? <GraduationCap className="w-4 h-4" /> : 
                  ...
                  <div className={`flex items-center gap-2 text-muted-foreground text-xs font-mono mb-4 ${idx % 2 === 0 ? "md:justify-end" : "md:justify-start"}`}>
                    <PixelCalendarIcon className="w-3 h-3" />
                    <span>{item.period}</span>
                    {'grade' in item && item.grade && (
                      <>
                        <span className="mx-2">•</span>
                        <span className="text-ctp-green font-bold">{item.grade}</span>
                      </>
                    )}
                  </div>
